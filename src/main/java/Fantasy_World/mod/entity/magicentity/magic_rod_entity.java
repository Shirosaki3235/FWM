...
public class magic_rod_entity extends Entity implements IProjectile{
	...
	public magic_rod_entity(World par1World, float size, double thisdamage, double thisrange, int thisCoolTime) {
		super(par1World);
		this.renderDistanceWeight = 10.0D;
			...
			if (entityList != null && !entityList.isEmpty()) {
				for (MovingObjectPosition target : entityList) {
					if (target != null && target.entityHit != null) {
						...
						if (target.entityHit instanceof IProjectile && !(target.entityHit instanceof EntityFireball)) {
